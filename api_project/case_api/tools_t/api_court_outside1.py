...
def ChargeFormat(charge):
    ...
    charge = re.sub(' |\t|\r|\n|　','', charge)
    pun_list = ['Title','PubDate','Html','"','{','}','nbsp']
...
class CourtETL:
	...
	def get_court(self, s,court_list):
		"""
		根据函数create_court_word_dict生成的字典，进行法院的提取
		"""
		char_to_court = self.create_court_word_dict(court_list)
		if not s:
			return ''
	...
	def FindChargeCourt(self,charge,court,max_substr_len):
		...
		if b != -1:
		    ...
		    while N<=L:
		        ...
		        if len(self.get_court(s,self.court_all)) == 1:
		        	return self.get_court(s,self.court_all)[0] 

		        if len(self.get_court(s,self.courtall_city)) == 1:
		        	return self.get_court(s,self.courtall_city)[0]
		        N +=1
		    return court
	...
	def is_court(self,court,charge):
	    ...
	    if len(self.get_court(court,self.court_all)) == 1:
	    	court2 = self.get_court(court,self.court_all)[0]
	    	return court2, self.province_dic[court2][0], self.province_dic[court2][1]

	    if len(self.get_court(court,self.courtall_city)) == 1:
	    	court3 = self.get_court(court,self.courtall_city)[0]
	...
	def norm_court(self,court,charge):
		...
		court_list = self.get_court(court_[0],self.court_all)  #进行最后的规范化
...
if __name__ == '__main__':
	...
	a = CourtETL()
	print(a.norm_court('十堰市竹溪县人民法院','十堰市竹溪县人民法院'))
	"""
	#定义读取文书数据库
